...
class Logger(object):
    ...
    def splitMulitChannel(self, t_list, max_save_n=8, gray=False):
        new_list = []
        if gray:
            for tensor in t_list:
                tensor = tensor[:max_save_n]
                if tensor.shape[1] > 1:
                    num = 1
                    new_list += torch.split(tensor, num, 1)
                else:
    ...
    def saveCoefResults(self, results, split, epoch, iters, nrow):
        pass
